        ...
        this.core = function() {
            ...
            const subscribe = function(endpoint, callback, reconnect = false, opened_callback = false) {
                ...
                ws.on('unexpected-response', function(req, res) {
                    ...
                    options.log('WebSocket Unexpected response: ');
                    if ( reconnect && options.reconnect ) {
                        options.log('..WebSocket reconnecting: ');
                        reconnect();
                    }
                    return true;
                });
            ...
            return {
                ...
                depthRequest:function(symbol, callback, limit = 100){
                    publicRequest(base+'v1/depth', {symbol:symbol, limit:limit}, callback);
                },
                    ...
                    depthCache: function depthCacheFunction(symbols, callback, limit = 500) {
                        ...
                        let getSymbolDepthSnapshot = function(symbol) {
                            publicRequest(base+'v1/depth', { symbol:symbol, limit:limit }, function(error, json) {
                                ...
                                if(context.messageQueue.filter !== undefined){
